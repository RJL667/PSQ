...
| 5j | **Recency clustering INSIDE the encrypted export** | Open. The on-demand encrypted CSV (`credential_export.py`, Manual §6.4) currently has no dates — only `email,username,password,hashed_password,database` — so the client cannot reproduce the dashboard's recency-band "clustering" from the file. Fix: add `breach_date` + `recency_band` columns by joining each record's `database` to the same per-source date-guesstimate the correlation card already uses (enriched_sources breach_date + `KNOWN_BREACH_DATES`). **Encryption is unaffected** — we encrypt whatever bytes we build, so enriching columns is orthogonal to age/AES. **Honest limitation:** DeHashed records carry no reliable per-credential date, so the band is per-SOURCE, not per-record (same caveat the recency timeline already has). The export re-queries DeHashed live and doesn't load the scan, so it must re-derive source dates at export time (or be passed the scan's `dehashed.enriched_sources`). |
| 5k | **Fresh IntelX / Hudson-Rock dump references — "most recent leaks/circulations" view (on client request)** | Open. The genuinely-fresh exposure (e.g. phishield's 30-90d band = 4 IntelX stealer-log postings, Mar-Apr 2026, incl. a SA-tagged victim log `48438_ZA_169.0.21.31` infected 2026-02-21) lives in the **IntelX channel and is NOT in the DeHashed export**. Goal: surface a date-ordered list of the most recent dump references so the client can act. Two tiers: (a) **listing-only (free)** — name/date/media/type we already capture in `intelx.recent_results`; (b) **content-fetch (costs IntelX credits, on explicit request)** — pull the dump body to confirm match TYPE, since `History/` = site merely visited whereas `Passwords/`+`Autofill/` = credential actually stolen (the free listing can't distinguish). Include these references in the encrypted export and/or report appendix. Pairs with [5j]. |
